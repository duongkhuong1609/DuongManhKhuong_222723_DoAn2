const sql=require('mssql')
const cfg={server:'localhost',instanceName:'SQLEXPRESS',database:'LAP_LICH_TU_DONG',authentication:{type:'default',options:{userName:'sa',password:'123456'}},options:{encrypt:false,trustServerCertificate:true}}
;(async()=>{
 const pool=await new sql.ConnectionPool(cfg).connect()
 const rs=(await pool.request().query(`
   SELECT TOP 120
     CAST(m.MaNganh AS NVARCHAR(50)) AS MaNganh,
     m.MaMon,m.TenMon,m.LoaiMon,TRY_CONVERT(INT,m.NamM) AS Nam,TRY_CONVERT(INT,m.HocKy) AS HocKy,
     COUNT(DISTINCT cm.MaGV) AS Experts
   FROM MON m
   LEFT JOIN CHUYEN_MON_CUA_GV cm ON cm.MaMon=m.MaMon
   WHERE CAST(m.MaNganh AS NVARCHAR(50)) IN ('3','4','5','6')
   GROUP BY m.MaNganh,m.MaMon,m.TenMon,m.LoaiMon,m.NamM,m.HocKy
   ORDER BY m.MaNganh, Experts ASC, m.MaMon
 `)).recordset||[]
 console.log(JSON.stringify(rs,null,2))
 await pool.close()
})().catch(e=>{console.error(e);process.exit(1)})
